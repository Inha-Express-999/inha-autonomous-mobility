...
namespace InhaExpress.Client.Tests
{
    public sealed class VehicleActorSpawnerTests
    {
        ...
        public void ServerSnapshotSpawnsActorWithConvertedPoseAndReporterIdOnlyOnce()
        {
            ...
            Assert.That(routeLine.positionCount, Is.EqualTo(3), "The displayed line starts at the actor and follows remaining route points.");
            Assert.That(Vector3.Distance(routeLine.GetPosition(0), actor.transform.position + Vector3.up * 0.12f),
                Is.LessThan(0.001f));
            Assert.That(Vector3.Distance(routeLine.GetPosition(2),
        ...
        public void NonDrivingSnapshotRevokesMotionWithoutDiscardingRouteProgress()
        {
            ...
            var routeLine = actor.GetComponentInChildren<LineRenderer>();
            Assert.That(routeLine.startColor, Is.EqualTo(new Color(1.00f, 0.60f, 0.10f, 0.95f)));
