...
public static class CampusBiryongSidewalk
{
    public static void Fit()
    {
        var landmark=UnityEngine.Object.FindObjectsByType<CampusLandmarkEntrances>(FindObjectsSortMode.None).Single(x=>x.landmarkId=="biryong_plaza");
        ...
        {
            float z=Mathf.Lerp(5f,7.5f,j/(float)columns);
            ...
            if(!found)throw new Exception("Biryong accessible sidewalk target missing.");
        ...
        const string path="Assets/CampusSim/Generated/EntranceApproaches/BiryongAccessibleLateral.asset";
        ...
        var area=child.GetComponent<CampusPedestrianArea>();area.areaId="biryong_plaza_accessible_lateral";area.localBoundary=polygon;area.excludeFromVehicleRouting=true;area.pedestrianAccessVerified=false;area.source="Synthetic sidewalk connection; measured access and Stop routing unverified.";
        ...
        File.WriteAllText("Docs/MapResearch/Iterations/biryong-accessible-lateral.txt",$"Width=2.5m; top samples={(rows+1)*(columns+1)}; maximum longitudinal grade={maxGrade}; terrain burial={maxBurial}; {columns+1} terminal samples gap={minGap}..{maxGap}m. Authoring geometry only.");
    }
}
