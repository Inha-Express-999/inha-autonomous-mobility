...
public static class CampusDormitoryThreeFrontage
{
    public static void Fit()
    {
        var landmark=UnityEngine.Object.FindObjectsByType<CampusLandmarkEntrances>(FindObjectsSortMode.None).SingleOrDefault(x=>x.landmarkId=="dorm_3");
        if(!landmark)return;
        ...
        const string path="Assets/CampusSim/Generated/EntranceApproaches/Dormitory3SharedFrontage.asset";
        var mesh=AssetDatabase.LoadAssetAtPath<Mesh>(path);if(!mesh){mesh=new Mesh();AssetDatabase.CreateAsset(mesh,path);}
        
        Undo.RecordObject(mesh,"Fit dormitory three frontage");
        ...
        var area=child.GetComponent<CampusPedestrianArea>();area.areaId="dormitory3_shared_frontage";area.localBoundary=polygon;area.excludeFromVehicleRouting=true;area.pedestrianAccessVerified=false;area.source="Synthetic sidewalk connection; measured access and Stop routing unverified.";
        ...
        File.WriteAllText("Docs/MapResearch/Iterations/dormitory3-shared-frontage.txt",$"Width=1.8m; top samples={(rows+1)*(columns+1)}; maximum longitudinal grade={maxGrade}; terrain burial={maxBurial}; {columns+1} terminal samples gap to accessible approach={minGap}..{maxGap}m. Authoring geometry only.");
    }
}
