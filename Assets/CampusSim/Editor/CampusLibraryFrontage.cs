...
public static class CampusLibraryFrontage
{
    public static void Fit()
    {
        var landmark=UnityEngine.Object.FindObjectsByType<CampusLandmarkEntrances>(FindObjectsSortMode.None).SingleOrDefault(x=>x.landmarkId=="library");
        if(!landmark)return;
        ...
        const string path="Assets/CampusSim/Generated/EntranceApproaches/LibrarySharedFrontage.asset";
        var mesh=AssetDatabase.LoadAssetAtPath<Mesh>(path);if(!mesh){mesh=new Mesh();AssetDatabase.CreateAsset(mesh,path);}
        
        Undo.RecordObject(mesh,"Fit library frontage");
        ...
        var area=child.GetComponent<CampusPedestrianArea>();area.areaId="library_shared_frontage";area.localBoundary=polygon;area.excludeFromVehicleRouting=true;area.pedestrianAccessVerified=false;area.source="Synthetic sidewalk connection; measured access and Stop routing unverified.";
        ...
        File.WriteAllText("Docs/MapResearch/Iterations/library-shared-frontage.txt",$"Width=1.8m; top samples={(rows+1)*(columns+1)}; maximum longitudinal grade={maxGrade}; terrain burial={maxBurial}; {columns+1} terminal samples gap to accessible approach={minGap}..{maxGap}m. Authoring geometry only.");
    }
}
