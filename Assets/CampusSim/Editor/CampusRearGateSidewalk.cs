...
public static class CampusRearGateSidewalk
{
    public static void Fit()
    {
        var landmark=UnityEngine.Object.FindObjectsByType<CampusLandmarkEntrances>(FindObjectsSortMode.None).Single(x=>x.landmarkId=="rear_gate");
        var portal=landmark.accessibleEntrance;
        var landing=portal.Find("Walkway");
        ...
        {
            float z=Mathf.Lerp(.7f,2.5f,j/(float)columns);
            bool found=false;float end=0,height=0;
            for(float x=start;x>=-7;x-=.01f)
            {
                var w=portal.TransformPoint(new Vector3(x,0,z));
                if(!paving.Raycast(new Ray(w+Vector3.up*100,Vector3.down),out var hit,200))continue;
                end=x;height=portal.InverseTransformPoint(hit.point).y+.005f;found=true;break;
            }
            if(!found)throw new Exception("Rear accessible sidewalk target missing.");
            maxGrade=Mathf.Max(maxGrade,Mathf.Abs(height-top)/(start-end));
        ...
        const string path="Assets/CampusSim/Generated/EntranceApproaches/RearAccessibleLateral.asset";
        ...
        var child=portal.Find("Lateral pedestrian connection");
        if(!child){child=new GameObject("Lateral pedestrian connection").transform;Undo.RegisterCreatedObjectUndo(child.gameObject,"Create lateral sidewalk");child.SetParent(portal,false);child.gameObject.AddComponent<MeshFilter>();child.gameObject.AddComponent<MeshRenderer>();child.gameObject.AddComponent<MeshCollider>();child.gameObject.AddComponent<CampusPedestrianArea>();}
        ...
        var area=child.GetComponent<CampusPedestrianArea>();area.areaId="rear_gate_accessible_lateral";area.localBoundary=polygon;area.excludeFromVehicleRouting=true;area.pedestrianAccessVerified=false;area.source="Synthetic sidewalk connection; measured access and Stop routing unverified.";
        ...
        File.WriteAllText("Docs/MapResearch/Iterations/rear-accessible-lateral.txt",$"Width=1.8m; top samples={(rows+1)*(columns+1)}; maximum longitudinal grade={maxGrade}; terrain burial={maxBurial}; {columns+1} terminal samples gap={minGap}..{maxGap}m. Authoring geometry only.");
    }
}
