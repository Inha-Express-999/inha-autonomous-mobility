...
public static class CampusStudentCenterFrontage
{
    public static void Fit()
    {
        var landmark=UnityEngine.Object.FindObjectsByType<CampusLandmarkEntrances>(FindObjectsSortMode.None).SingleOrDefault(x=>x.landmarkId=="student_center");
        if(!landmark)return;
        ...
        {
            float z=Mathf.Lerp(2.25f,4.05f,j/(float)columns);
        ...
        const string path="Assets/CampusSim/Generated/EntranceApproaches/StudentCenterSharedFrontage.asset";
        var mesh=AssetDatabase.LoadAssetAtPath<Mesh>(path);if(!mesh){mesh=new Mesh();AssetDatabase.CreateAsset(mesh,path);}
        
        Undo.RecordObject(mesh,"Fit student center frontage");
        ...
        var area=child.GetComponent<CampusPedestrianArea>();area.areaId="student_center_shared_frontage";area.localBoundary=polygon;area.excludeFromVehicleRouting=true;area.pedestrianAccessVerified=false;area.source="Synthetic sidewalk connection; measured access and Stop routing unverified.";
        ...
        File.WriteAllText("Docs/MapResearch/Iterations/student-center-shared-frontage.txt",$"Width=1.8m; top samples={(rows+1)*(columns+1)}; maximum longitudinal grade={maxGrade}; terrain burial={maxBurial}; {columns+1} terminal samples gap to accessible approach={minGap}..{maxGap}m. Authoring geometry only.");
    }
}
