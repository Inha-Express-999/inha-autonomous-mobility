...
public static class CampusLibraryPavingConnection
{
    public static void Fit()
    {
        var landmark=UnityEngine.Object.FindObjectsByType<CampusLandmarkEntrances>(FindObjectsSortMode.None).SingleOrDefault(l=>l.landmarkId=="library");
        if(!landmark)return;
        ...
        {
            // Skewed seams retain a perpendicular ribbon width of about 1.8m.
            float offset=Mathf.Lerp(-1.45f,1.45f,j/(float)columns);
            var start=p.TransformPoint(new Vector3(offset,0,7.98f));
            if(!approach.Raycast(new Ray(start+Vector3.up*100,Vector3.down),out var first,200))throw new InvalidOperationException("Library approach seam missing.");
            float z=20;RaycastHit last=default;
            for(;z<25;z+=.01f)
            {
                var target=p.TransformPoint(new Vector3(18+offset,0,z));
                if(paving.Raycast(new Ray(target+Vector3.up*100,Vector3.down),out last,200))break;
            }
            if(z>=25)throw new InvalidOperationException("Library paving seam missing.");
            for(int i=0;i<=rows;i++)
            {
                var world=Vector3.Lerp(first.point,last.point+Vector3.up*.005f,i/(float)rows);
                clearance=Mathf.Min(clearance,world.y-terrain.SampleHeight(world)-terrain.transform.position.y);
                if(obstacles.Any(c=>c.Raycast(new Ray(world+Vector3.up*100,Vector3.down),out _,200)))throw new InvalidOperationException("Library connection crosses road or building.");
                vertices.Add(p.InverseTransformPoint(world));
            }
        }
        if(clearance<.01f)throw new InvalidOperationException("Library connection needs ground clearance: "+clearance);
        float maxGrade=0;
        ...
        const string path="Assets/CampusSim/Generated/EntranceApproaches/LibraryPavingConnection.asset";
        var mesh=AssetDatabase.LoadAssetAtPath<Mesh>(path);if(!mesh){mesh=new Mesh();AssetDatabase.CreateAsset(mesh,path);}
        Undo.RecordObject(mesh,"Fit library paving connection");mesh.Clear();mesh.SetVertices(vertices);mesh.SetTriangles(triangles,0);mesh.RecalculateNormals();mesh.RecalculateBounds();EditorUtility.SetDirty(mesh);
        var child=p.Find("Lateral pedestrian connection");
        if(!child){child=new GameObject("Lateral pedestrian connection").transform;Undo.RegisterCreatedObjectUndo(child.gameObject,"Create library paving connection");child.SetParent(p,false);child.gameObject.AddComponent<MeshFilter>();child.gameObject.AddComponent<MeshRenderer>();child.gameObject.AddComponent<MeshCollider>();child.gameObject.AddComponent<CampusPedestrianArea>();}
        ...
        var area=child.GetComponent<CampusPedestrianArea>();area.areaId="library_paving_connection";area.localBoundary=polygon;area.excludeFromVehicleRouting=true;area.pedestrianAccessVerified=false;area.source="Synthetic diagonal walkway to existing scene pedestrian network; actual access and Stop unverified.";
        ...
        File.WriteAllText("Docs/MapResearch/Iterations/library-paving-connection.txt",$"Top vertices=485; max face gradient={maxGrade}; minimum vertex terrain clearance={clearance}; endpoint paving offset=0.005m; access unverified.");
    }
}
