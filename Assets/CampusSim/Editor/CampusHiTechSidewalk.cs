...
public static class CampusHiTechSidewalk
{
    public static void Fit()
    {
        var landmark=UnityEngine.Object.FindObjectsByType<CampusLandmarkEntrances>(FindObjectsSortMode.None).Single(x=>x.landmarkId=="hitech");
        var portal=landmark.generalEntrance;
        var landing=portal.Find("Threshold");
        ...
        for(int j=0;j<=columns;j++)
        {
            ...
            if(!found)throw new Exception("HiTech general sidewalk target missing.");
        ...
        const string path="Assets/CampusSim/Generated/EntranceApproaches/HiTechGeneralLateral.asset";
        ...
        var area=child.GetComponent<CampusPedestrianArea>();area.areaId="hitech_general_lateral";area.localBoundary=polygon;area.excludeFromVehicleRouting=true;area.pedestrianAccessVerified=false;area.source="Synthetic sidewalk connection; measured access and Stop routing unverified.";
        ...
        File.WriteAllText("Docs/MapResearch/Iterations/hitech-general-lateral.txt",$"Width=2.5m; top samples={(rows+1)*(columns+1)}; maximum longitudinal grade={maxGrade}; terrain burial={maxBurial}; {columns+1} terminal samples gap={minGap}..{maxGap}m. Authoring geometry only.");
    }
}
