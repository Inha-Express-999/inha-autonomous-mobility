...
**문서:** v0.1.0.2 · 2026-09-17 · 요구사항/설계/구현/검증 통합본
...
**화면 방향은 모바일 세로(Portrait), PC 가로(Landscape)로 고정된 설계 요구사항이다.** 모바일은 세로 화면 기준 3D 지도·상단 상태·하단 호출/운행 안내를 배치하고 Safe Area·노치·소프트 키보드·다양한 세로 화면비를 검증한다. 자동 가로 전환은 허용하지 않는다. PC는 가로 화면에서 중앙 지도와 관제 패널을 구성하고 창 크기/해상도 변경에도 입력과 지표가 잘리지 않게 한다. 공통 Unity 프로젝트의 빌드 프로파일/빌드 단계에서 플랫폼별 방향·해상도·품질을 명시적으로 적용해 서로의 설정을 덮어쓰지 않는다. T18에는 모바일 세로 실기기와 PC 가로 화면의 UI/카메라·가독성 검증을 포함한다.

**건물 외형 변경 전에는 해당 실제 건물 사진을 확인해야 한다.** 공식 대학 자료·직접 촬영·사용자 제공 사진을 우선하고 출처 URL/촬영 또는 확인 시점/대상 건물·시점을 기록한다. 정면과 전경을 우선 확보하고 가능한 경우 측면도 확인해 매스·층 구분·지붕·창 배열·주요 색상·출입구를 실제 디자인에 가깝게 수정한다. 사진 원근만으로 실제 치수를 확정하지 않는다. 미세 장식/실내는 생략 가능하지만 임의의 공통 박스 건물로 대표 시설의 특징을 대체하지 않는다. 사진이 없는 부분은 미확인/근사로 표시한다. 수정 후 같은 방향의 Unity 화면과 참고 사진을 비교해 건물별 결과를 남긴다. 사진 자체의 텍스처 재배포는 별도 사용 권한을 확인한다. 대략적 형태라는 목표는 디테일 수준을 줄이는 뜻이며 실제 건물 디자인을 무시하는 뜻이 아니다.
