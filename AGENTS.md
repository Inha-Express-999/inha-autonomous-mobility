...
**문서:** v0.2.0.0 · 2026-09-17 · 요구사항/설계/구현/검증 통합본  
...
기존 문서 v1.0~v1.2는 네 자리 규칙 도입 전 문서 개정 번호이며 완성/공개 버전이 아니다. 이번 통합 설계 기준선을 **`0.2.0.0`**으로 전환한다. 이는 구현 완료를 뜻하지 않으며 후속 변경부터 위 규칙으로 증가시킨다.
...
필수 초기 목록은 사용자 지정 명칭인 **정문, 지하철역, 후문, 5호관, 2호관, 하이테크, 60주년, 비룡플라자, 기숙사 1·2·3·4**다. 기숙사는 네 개의 개별 랜드마크로 관리한다. 이는 검증 전 요구 목록이며 역명/출구, 기숙사 번호와 실제 시설의 대응, 공식 시설명·좌표·출입구는 공식 캠퍼스 지도와 현장 확인으로 확정한다. 존재나 위치가 확인되지 않은 항목을 임의로 생성하지 않는다.
