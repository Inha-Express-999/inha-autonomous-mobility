...
**문서:** v0.4.2.0 · 2026-09-26 · 요구사항/설계/구현/검증 통합본
...
2026-09-26 v0.4.2.0 추가: 선택적 합성 정차 충전 설정, FIFO 슬롯·완료/중단 복구·위치 확인 후 해제를 검증했다. 전용 11개 및 전체 Python/MapData 467개 통과. 충전소 자동 이동·물리 dock/예약 통합·Unity/Player와 전체 MVP는 미완료다.

2026-09-26 v0.4.1.0 추가: disjoint CBS 비교와 비동기 제안 worker, 경로 비용 기반 Greedy/Hungarian 비교, 선택적 합성 에너지 배차 제한을 추가했다. 충전 슬롯 lifecycle은 기본 비활성·전용 테스트 없는 작성 중 코드로 보존한다. Python/MapData 456개 및 Ruff 통과. CBS/RRT live 실행, 실제 지도, 충전 통합, 최신 Unity/Player/Android와 전체 MVP는 미완료다.

2026-09-26 v0.4.0.0 추가: 자원 진입/해제 판정을 수정하고 합성 세 차량 승객·화물 운송 및 점유 중 telemetry 중단 검증을 추가했다. PC/모바일 통로 대기 사유와 ETA 미확인 표시는 갱신된 클라이언트가 필요하다. 오프라인 CBS 비교 기반은 추가했으나 세 차량 통로 benchmark에서 탐색 한도를 초과하며 실행 권한이 없다. Python/MapData 374개와 Ruff 통과. Unity 기록은 각 실행 소스에 한정하며 전체 MVP는 미완료다.

2026-09-26 v0.3.2.0 추가: LiDAR ray별 HIT/MISS/INVALID 계약, 관측 운동 기반 시간 궤적 검사와 실행 권한 없는 동적 RRT 후보, 선택적 자체 D* Lite 및 합성 비용 비교, 자원 예약·ego 점유 추적·Python 속도 제한을 추가했다. 실제 지도와 다중 차량 Physics/CBS 및 전체 MVP는 미완료다. 커밋 검증 범위는 CHANGELOG.md와 Docs/MVP_EXECUTION.md를 따른다.

2026-09-26 v0.3.1.0 추가: 센서 기반 횡단 정지와 선택적 Python 제어 명령/Unity actuator, 명령 만료 제동·actor 재생성·동일 서버 재접속을 검증했다. 자체 RRT 후보와 연속 footprint/시간 궤적 검사 기반을 추가했으나 RRT 후보 실행 권한은 없다. Python/MapData 242개, Unity component PlayMode 12/12 및 socket 중단 포함 통합 1/1 통과 기록이 있다. 최신 기능의 Player/Android·원본 씬·실제 지도·서버 프로세스 재시작과 전체 MVP는 미검증 또는 미완료다. 실행별 소스와 한계는 Docs/MVP_EXECUTION.md를 따른다.

2026-09-26 v0.3.0.0 추가: 명시적인 차량 telemetry 소유권과 재생성 tick 연속성, 합성 보행자 population, Raycast Radar 접근 정지, 명시적 zone 폐쇄/재개, 관측 ID·취득 시각·센서 pose 및 보행자 관측 운동 추정을 추가했다. 관측 운동은 진단용이며 full TTC 제어는 미연결이다. 새 센서 payload는 갱신된 서버와 함께 사용해야 한다. Python/MapData 178개 및 Ruff 통과; Unity 증거는 artifacts/validation의 실행별 소스에 한정한다. 실제 지도와 전체 MVP는 미완료다.
...
기존 문서 v1.0~v1.2는 네 자리 규칙 도입 전 문서 개정 번호이며 완성/공개 버전이 아니다. 2026-09-17 맵 수정 전 체크포인트를 `0.1.0.0`으로 재설정한 이력은 유지하되, 이후 클라이언트 구조·DTO·fixture·uGUI 구현, 알고리즘 비교 설계 보완과 커밋별 버전/description 관리 규칙을 반영해 현재 문서 기준은 **`0.4.2.0`**이다. 과거 버전은 소급 변경하지 않는다.
