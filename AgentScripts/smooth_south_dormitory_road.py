...
import json
out=Path('Docs/MapResearch/Iterations/SouthDormitoryRoad');n=1025
...
def mask(path):
 tris=np.fromfile(path,'<f4').reshape(-1,3,2);polys=[]
 for t in tris:
  a=t[1]-t[0];b=t[2]-t[0]
  if abs(a[0]*b[1]-a[1]*b[0])>.001:polys.append(({'type':'Polygon','coordinates':[[*t.tolist(),t[0].tolist()]]},1))
 return rasterize(polys,out_shape=(n,n),transform=Affine(2,0,-901,0,2,-901),all_touched=True).astype(bool)
base=Path('Docs/MapResearch/Iterations/SportsRoadGrading')
protected=mask(out/'protected-xz.raw')

roadmask=mask(base/'FineTerrain/all-roads-xz.raw')
free=(x>530)&(x<620)&(z>-500)&(z<-335)&~protected
...
for j,i in np.argwhere(free):
 ...
 for dj,di in [(1,0),(0,1)]:
  road=roadmask[j,i] and roadmask[j+dj,i+di]
...
# Keep ten metres at the boundary unchanged, then blend over twenty metres.
distance=np.minimum.reduce([x-530,620-x,z+500,-335-z])
t=np.clip((distance-10)/20,0,1);t=t*t*(3-2*t)
(before+(after-before)*t).astype('<f4').tofile(out/'transition-proposal.raw')
