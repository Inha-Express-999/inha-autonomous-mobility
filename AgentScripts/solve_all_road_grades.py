...
s['before']=np.fromfile(s['OUT']/'FineTerrain/road-grade-before.raw','<f4').reshape(1025,1025)
...
for line in (out/'FineTerrain/EasternFootprints/manifest.txt').read_text(encoding='utf-8-sig').splitlines():
    ...
    for index in cells[1:]:constraint([(index,1),(cells[0],-1)],0,0)
plaza=approach_mask('biryong_plaza_')
for j,i in np.argwhere(plaza):constraint([(indices[j,i],1)],2.207899,2.207899)
groups.append(('Biryong plaza entrance approach pads',plaza))
...
# Never create a steeper transition at the boundary of a protected surface.
# Include water-overlapping road support cells in this regression guard.
all_support = dilate(s['mask'](out/'FineTerrain/current-road-footprint.raw')) & interior
for j,i in np.argwhere(all_support):
    for dj,di in ((0,1),(1,0)):
        if indices[j+dj,i+di] < 0: continue
        limit=max(.09, abs(height[j,i]-height[j+dj,i+di])+.0001)
        constraint([(indices[j,i],1),(indices[j+dj,i+di],-1)],-limit,limit)
...
if result.info.status=='solved':
    ...
    after.tofile(out/'FineTerrain/road-grade-proposal.raw')
(out/'FineTerrain/road-grade-report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
