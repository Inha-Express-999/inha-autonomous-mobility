...
import json
out=Path('Docs/MapResearch/Iterations/EastRoadCoupled');n=1025
...
protected=mask(base/'water-triangles.raw')|mask(out/'pedestrian-xz.raw')
for path in (base/'FineTerrain/EasternFootprints').glob('building_*.raw'):protected|=mask(path)
roadmask=mask(base/'FineTerrain/all-roads-xz.raw')
free=(x>200)&(x<290)&(z>-185)&(z<-25)&~protected
