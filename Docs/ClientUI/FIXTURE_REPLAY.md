...
3. 상단 `FIXTURE DATA`를 확인한다. PC는 전체 차량/요청/Zone 대시보드, 모바일은 fixture-passenger의 요청/배정 차량/Stop 하단 시트를 표시한다.
4. Editor/Development Build의 `일시정지`, `다시 시작`, `연결 끊기`로 상태를 확인한다. 모바일에도 있는 이 버튼들은 개발용 로컬 재생 조작이며 승객용 서버 조작이 아니다. 비개발 빌드에는 표시하지 않는다.
5. 연결을 복구하려면 `다시 연결`을 누른다. 수신 중단 1초 후 `상태 지연`을 표시하고 기존 ETA/상태를 유지하며, 복구 시 최신 전체 상태로 바뀐다.
...
- 한국어 uGUI fixture 화면이며 완성된 사용자 UI가 아니다. Fixture·Zone·Stop·ETA·ID 같은 기술 용어는 영어를 유지한다. snapshot 변경 때 역할 내용을 갱신하고 연결 표시는 최대 10Hz, CanvasScaler와 Safe Area를 적용한다. 키보드/컨트롤러 이동·실기기 터치 검증은 별도다.
...
- Unity 6000.3.21f1 컴파일 성공, EditMode 54/54, uGUI 생성·버튼·Presenter 수명주기 PlayMode 1/1 통과.
- PC 역할에서 단일 host/view, 전체 차량 3대·요청 2개, stale/복구/새 run 재시작을 확인했다. 모바일 역할에서 자기 요청과 배정 차량만 전달되고 Zone이 제외되는 것을 확인했다.
- PC 1440×900과 모바일 Game View 720×1280에서 화면을 캡처해 겹침과 잘림을 확인했다. 실제 uGUI 버튼 클릭으로 pause/restart/disconnect/reconnect 상태 전환을 확인했다. 노치/Safe Area 실단말, 터치 조작, 성능 측정 결과는 아니다. 모바일 검증은 Windows Editor에서 모바일 역할 씬을 Additive로 미리 연 상태로 수행했다.
