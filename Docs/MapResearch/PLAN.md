...
프로젝트 기준: 0.1.0.2 · 조사일: 2026-09-17 · 상태: 계획/데이터 준비, Unity 씬 미변경
...
건물별 `참고 사진 출처 → 관찰한 특징 → 수정한 부분 → 비교 화면 → 미확인 부분` 기록을 작성한다. 공식 자료/사용자 사진을 우선 사용하고 다른 시기의 증축·리모델링 사진을 혼용하지 않도록 확인 시점을 기록한다.

1. 본관·60주년기념관·하이테크센터·정석학술정보관의 정면/전경을 먼저 확인한다.
2. 2호관·5호관·비룡플라자·제1~3생활관과 나머지 대표 시설의 사진을 대조한다.
3. 실제 사진에서 확인한 매스 비율, 고층/저층부, 지붕 형태, 창문 띠/배열, 주재료 색, 기둥/입구 등 식별 특징을 모델에 반영한다.
4. 실제 배치/중정/출입구를 유지하고 Flat Kit로 색감과 명암을 통일한다. 스타일 통일을 이유로 건물 고유 형태를 없애지 않는다.
5. 동일 시점의 Unity 캡처와 사진으로 확인한다. 정밀 치수·실내·작은 장식은 요구하지 않는다. 자료 없는 면과 높이는 근사임을 기록한다.

현재 전체 건물의 사진 세트 확인은 미완료다. 사진 확인 없이 외형 개선 완료로 표시하지 않는다. 이 절차는 이후 실제 씬 수정의 필수 선행 작업이다.
...
| 항목 | 모바일 승객 클라이언트 | PC 관제 클라이언트 |
|---|---|---|
| 방향 | 세로 Portrait, 자동 가로 전환 금지 | 가로 Landscape 중심 |
| 구성 | 상단 연결/요청 상태, 중앙 3D 지도, 하단 호출·운행 안내 카드 | 중앙 전체 3D 지도, 측면 요청/차량 패널, 시간 제어·지표 영역 |
| 입력 | 터치, 랜드마크 검색/목록, 확대·이동, 내 차량 따라보기 | 마우스/키보드, 전체 관제·시나리오 제어 |
| 표시 | 자신의 차량·경로·승하차 출입구·ETA·서버 판단 안내 | 전체 차량·군중·heatmap·전역/지역 경로·배차·배터리·이유 |
| 검증 | Safe Area, 노치, 키보드 표시, 긴 한글, 여러 세로 화면비, 실제 단말 | 가로 화면비·창 리사이즈·해상도 변화, 패널/지표 잘림 |

공통 CampusWorld와 서버 상태를 사용하되 UI·카메라 프레이밍·품질·빌드 설정은 역할별로 분리한다. 모바일용 orientation 설정을 PC 빌드에 그대로 적용하지 않는다. 실제 프로파일 구현 및 실기기 검증은 후속 UI 단계이며, 이번 계획 보완으로 완료된 것이 아니다.
